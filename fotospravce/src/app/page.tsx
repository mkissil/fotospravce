...
const features = [
  { icon: Camera, title: 'Zakázky v pohybu', desc: 'Pipeline, který ukazuje, co je poptávka, co čeká na podpis a co už míří do galerie.' },
  { icon: Users, title: 'Klienti po ruce', desc: 'Kontakty, poznámky, historie i firmy bez přepínání do dalších nástrojů.' },
  { icon: FileText, title: 'Faktury a smlouvy', desc: 'Přímo navázané na zakázku, takže i bez čtení je jasné, kde práce pokračuje.' },
  { icon: ImageIcon, title: 'Galerie pro výběr', desc: 'Náhledy fotek a klientský výběr jsou přirozenou součástí stejného flow.' },
];

const proof = [
  { value: '1', label: 'systém místo pěti různých nástrojů' },
  { value: 'CZ', label: 'prostředí a logika pro české fotografy' },
  { value: '14 dní', label: 'na vyzkoušení zdarma bez karty' },
...
const painPoints = [
  ...
  { icon: Smartphone, title: 'Galerie a kalendář žijí jinde' },
];

const plans = [
  { name: 'Začátečník', price: 249, yearlyPrice: 199, features: ['50 klientů', '10 zakázek / měsíc', 'QR faktury', '2 GB galerie'] },
  { name: 'Profesionál', price: 449, yearlyPrice: 359, popular: true, features: ['Neomezené zakázky', 'Smlouvy s podpisem', 'Galerie s výběrem', 'Automatizace a reporty'] },
  { name: 'Studio', price: 799, yearlyPrice: 639, features: ['Branding galerií', 'Prioritní podpora', '50 GB úložiště', 'Pokročilé exporty'] },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden">
      <motion.header className="sticky top-0 z-50 border-b border-white/45 bg-[rgba(251,246,239,0.7)] backdrop-blur-xl" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
          <Link href="/" className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,var(--accent),#ff946b)] text-sm font-bold text-white shadow-[0_18px_40px_-24px_rgba(214,93,56,0.95)] sm:h-11 sm:w-11 sm:text-base">F</div>
            <div>
              <p className="truncate font-serif text-lg font-semibold sm:text-xl">Foto<span className="text-[var(--accent)]">Správce</span></p>
              <p className="hidden text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--text-muted)] sm:block">Studio OS pro fotografy</p>
            </div>
          </Link>
          ...
          <nav className="hidden items-center gap-7 md:flex">
            <a href="#produkt" className="text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)]">Produkt</a>
            <a href="#cenik" className="text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)]">Ceník</a>
            <Link href="/login" className="text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)]">Přihlásit se</Link>
          ...
          <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <motion.div initial="initial" animate="animate" variants={stagger}>
              ...
              <motion.p variants={staggerItem} className="mt-5 max-w-2xl text-base leading-7 text-[var(--text-secondary)] sm:mt-6 sm:text-xl sm:leading-8">
                Klient, termín, smlouva, faktura, galerie. FotoSprávce skládá celý workflow fotografa do jedné vizuálně silné vrstvy, která je čitelná na první pohled.
              </motion.p>
              <motion.div variants={staggerItem} className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
                <Link href="/register" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,var(--accent),#ff9369)] px-7 py-4 text-base font-semibold text-white shadow-[0_26px_58px_-28px_rgba(214,93,56,0.95)] hover:-translate-y-0.5 sm:w-auto">
                  Vyzkoušet 14 dní zdarma <ArrowRight size={18} />
                </Link>
              ...
              <motion.div variants={staggerItem} className="mt-7 flex gap-3 overflow-x-auto pb-1 sm:mt-8 sm:flex-wrap">
                {['Klienti', 'Zakázky', 'Smlouvy', 'Faktury', 'Galerie'].map((chip) => (
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-3">
                    {[
                      ...
                      { label: 'Galerie', value: '6' },
                        ...
                        <p className="mt-2 text-2xl font-bold font-mono">{item.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
                  <div className="space-y-4">
                    <div className="surface-panel rounded-[24px] p-4 sm:rounded-[28px]">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--text-muted)]">Galerie po focení</p>
                          <p className="mt-1 text-sm font-semibold text-[var(--text)]">Výběr pro klienta</p>
                        </div>
                        <ImageIcon size={18} className="text-[var(--accent)]" />
                      </div>
                      <div className="mt-4 grid grid-cols-3 gap-3">
                        {['Wedding', 'Portrait', 'Brand'].map((frame, index) => (
                          <div key={frame} className="overflow-hidden rounded-[18px] border border-white/70 bg-white/80 sm:rounded-[22px]">
                            <div className={`aspect-[4/5] ${index === 0 ? 'bg-[linear-gradient(160deg,#2a211e,#9d6b55)]' : index === 1 ? 'bg-[linear-gradient(160deg,#3c425b,#c9ad95)]' : 'bg-[linear-gradient(160deg,#5a3828,#f0cfb6)]'}`} />
                            <p className="px-3 py-2 text-xs font-semibold text-[var(--text)]">{frame}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="surface-panel rounded-[24px] p-4 sm:rounded-[28px]">
                      ...
                      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                        {['Poptávka', 'Nabídka', 'Smlouva', 'Focení', 'Galerie'].map((stage, index) => (
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      { icon: ImageIcon, label: 'Galerie · výběr', status: 'Sdíleno', tone: 'bg-[var(--success)]' },
                    ].map((item) => (
                    ...
                    <div className="surface-panel-dark rounded-[24px] p-4 text-white sm:rounded-[28px]">
                      ...
                      <p className="mt-2 text-3xl font-bold font-mono">47 500 Kč</p>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-10 flex justify-center">
              <div className="surface-panel inline-flex w-full items-center justify-center gap-2 rounded-full p-1.5 sm:w-auto">
                <button onClick={() => setYearly(false)} className={`rounded-full px-4 py-2 text-sm font-semibold ${!yearly ? 'bg-[var(--text)] text-white' : 'text-[var(--text-secondary)]'}`}>Měsíčně</button>
                <button onClick={() => setYearly(true)} className={`rounded-full px-4 py-2 text-sm font-semibold ${yearly ? 'bg-[var(--text)] text-white' : 'text-[var(--text-secondary)]'}`}>Ročně</button>
              </div>
            </div>
            <div className="mt-12 grid gap-5 lg:grid-cols-3">
              {plans.map((plan) => (
      ...
      <footer className="border-t border-white/45 bg-white/40 px-4 py-12 sm:px-6">
        <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-4">
          <div>
            <p className="font-serif text-2xl font-semibold text-[var(--text)]">Foto<span className="text-[var(--accent)]">Správce</span></p>
            <p className="mt-3 max-w-xs text-sm leading-7 text-[var(--text-secondary)]">Studio OS pro české fotografy. Klienti, zakázky, faktury, smlouvy a galerie v jednom čitelném systému.</p>
          </div>
          <div>
